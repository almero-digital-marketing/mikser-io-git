...
// Core's own checksum, not a second implementation of it: the value being
// compared was produced by this exact function at write time, and two
// hashes that merely agree today would drift the first time either moved.
import { checksum as fileChecksum } from 'mikser-io'
...
//   no-diff      the paths are on disk and identical to the last commit, and
//                they still hold the bytes this set wrote — so the write was
//                a genuine no-op against what was already committed
//   replaced     they are on disk, identical to the last commit, and NOT what
//                this set wrote. Something overwrote it before the pass ran.
//                This is lost work, and it used to be indistinguishable from
//                the line above
//   unverifiable same shape, but the writer recorded no checksum, so which of
//                the two it is cannot be established. Named rather than
//                guessed at
...
// like routine.
async function classifySettled(folder, set, scoped) {
    ...
    if (gone.length) return gone.length === writes.length ? 'paths-gone' : 'partly-gone'

    // Present, and identical to HEAD. Whether that is a no-op or a loss is
    // decided by comparing what is there against what this set WROTE — the
    // only evidence that separates them, and the reason it is recorded.
    const written = set.checksums ?? {}
    let verified = 0
    for (const rel of writes) {
        const expected = written[rel]
        if (!expected) continue
        verified++
        if (await fileChecksum(path.join(folder, rel)) !== expected) return 'replaced'
    }
    return verified === writes.length ? 'no-diff' : 'unverifiable'
}
...
export const SETTLED_WITHOUT_COMMIT = new Set([
    'empty', 'deleted', 'no-diff', 'replaced', 'unverifiable', 'paths-gone', 'partly-gone',
])
...
export async function commitAndPushWriteBranch(
    ...
    if (!porcelain) {
        ...
        for (const set of changeSets) {
            await onSettled?.(set.id, await classifySettled(folder, set, withinPaths(set.paths, paths)))
        }
    ...
    for (const set of changeSets) {
        ...
        try {
            try {
                ...
                await onSettled?.(set.id, await classifySettled(folder, set, scoped))
                continue
            }
