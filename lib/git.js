...
export async function statusPorcelain(folder) {
    ...
    // 3-file new directory reported as a single line). `git add -A`
    // itself was never affected by this — it always stages everything
    // regardless of how status reports it — this only fixes what the
    // commit message claims happened.
    return run(folder, ['status', '--porcelain', '--untracked-files=all'])
}
