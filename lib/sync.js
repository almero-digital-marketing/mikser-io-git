...
export async function commitAndPushWriteBranch(folder, { writeBranch, message, author, token }) {
    const porcelain = await git.statusPorcelain(folder)
    if (!porcelain) return { committed: false, pushed: false }
    const fileCount = porcelain.split('\n').filter(Boolean).length

    await git.addAll(folder)
    if (!(await git.hasStagedChanges(folder))) {
