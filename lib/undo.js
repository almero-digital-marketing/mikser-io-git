...
export async function previewUndo(folder, { id, branch, runtime } = {}) {
    ...
    // made one.
    if (set.outcome === 'empty' || set.outcome === 'out-of-scope') {
        return {
            ok: false,
            refused: set.outcome === 'empty' ? 'nothing-to-undo' : 'out-of-scope',
            set,
            error: set.outcome === 'empty'
                ? 'That change set left nothing on disk — its own changes cancelled out, so no commit was made '
                    + 'and there is nothing to take back. The files are already as they were.'
