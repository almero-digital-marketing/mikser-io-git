...
export async function previewUndo(folder, { id, branch, runtime } = {}) {
    ...
    if (!set) {
        return { ok: false, refused: 'unknown-change-set',
                 error: `No change set ${id} in the log. Ids come from mikser_changes; the log keeps the most `
                     + 'recent changes, so a very old one may have aged out.' }
    }
    ...
    if (!set.recordedAs) {
        return {
            ...
            error: 'That change is recorded but has not been committed yet, so there is nothing to revert from. '
