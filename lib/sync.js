...
export async function commitAndPushWriteBranch(
    ...
    for (const set of changeSets) {
        ...
        committedAny = true
        // The sha is what an undo reverts FROM. Reported back so the engine's
        // log can say what each set maps to, which is the difference between
        // "no such change set" and "recorded, but nothing has committed it".
