...
import { SETTLED_WITHOUT_COMMIT } from './sync.js'
...
export async function previewUndo(folder, { id, branch, runtime } = {}) {
    ...
    // Finished with no commit, because its changes cancelled out. Not
    // pending, and undoing it is a different operation from undoing one that
    // produced a commit: there is no contribution to remove, because it never
    // made one.
    if (SETTLED_WITHOUT_COMMIT.has(set.outcome) || set.outcome === 'out-of-scope') {
        // The refusal is the same — there is no commit to revert — but the
        // reason is not, and telling someone "its changes cancelled out" about
        // a write that went missing sends them away satisfied.
        const gone = set.outcome === 'paths-gone' || set.outcome === 'partly-gone'
        return {
            ok: false,
            refused: set.outcome === 'out-of-scope' ? 'out-of-scope'
                : gone ? 'nothing-to-undo-work-missing'
                : 'nothing-to-undo',
            set,
            error: set.outcome === 'out-of-scope'
                ? 'That change set wrote outside the folders this instance versions, so it has no commit here. '
                    + 'Whatever manages those paths is what can take it back.'
                : gone
                ? 'That change set wrote files that were no longer on disk when the commit pass ran, so nothing '
                    + 'was committed and there is nothing to take back — but its work is not in git either. '
                    + 'It was lost rather than cancelled. Re-apply it from whatever asked for it.'
                : 'That change set left nothing on disk to commit — either it wrote what was already there, or '
                    + 'something replaced it before the pass ran. Either way there is no commit to take back.',
        }
    }
    if (!set.recordedAs) {
        return {
