...
//
// 0 means the target already contains everything on the write branch, so
// there is nothing to promote. Anything else means the pipeline has work
// outstanding — which is the condition worth retrying on, and the one worth
// shouting about if it persists. Asked of the remote-tracking refs, because
// what matters is what the forge can see, not what is checked out here.
export async function commitsAhead(folder, ref, base) {
    const out = await run(folder, ['rev-list', '--count', `${base}..${ref}`])
