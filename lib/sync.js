...
export async function commitAndPushWriteBranch(
    folder, { paths, writeBranch, message, author, token, changeSets = [], reserved = [], onCommitted, onFailed, onSettled },
) {
    ...
    if (!porcelain) {
        ...
        for (const set of changeSets) onSettled?.(set.id, 'empty')
